...
void physicalSwitch(){
	...
	//Lit et stock l'état du switch
	switch_Nunchuck_Joystick = HAL_GPIO_ReadPin(SLIDER_GPIO_Port, SLIDER_Pin);

	static bool old_switch_Nunchuck_Joystick = false;
...
int main(void)
{
  ...
  */
  ...
  XF_post(steeringProcess, E_INIT_STEERING, 2500);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : PushButton_Pin SLIDER_Pin */
  GPIO_InitStruct.Pin = PushButton_Pin|SLIDER_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
